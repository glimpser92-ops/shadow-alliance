...
function calculateRoundResult(room, round) {
  ...
  const penalty = winnerTeam ? getPenalty(winningSubmissions) : null;
...
function getPenalty(winningSubmissions) {
  if (winningSubmissions.length <= 1) return null;
  const maxPenaltyTarget = Math.max(1, Math.floor(winningSubmissions.length / 2));
  const targetCount = randomBetween(1, maxPenaltyTarget);
