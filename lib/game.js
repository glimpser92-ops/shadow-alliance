...
function countTeams(room) {
  return Object.values(room.players).reduce(
    (counts, player) => {
...
function joinRoom(room, existingPlayerId) {
  ...
  const usedNames = new Set(Object.values(room.players).map((player) => player.nickname));
  const player = {
    id,
...
function getFinalResult(room) {
  ...
  return {
    ...
    rosters: {
      black: Object.values(room.players)
        .filter((player) => player.team === "black")
        .map(publicPlayer),
      white: Object.values(room.players)
...
function serializeRoom(room, options = {}) {
  ...
  const submittedIds = new Set(Object.keys(activeRound?.submissions || {}));
  const personalPlayer = playerId ? room.players[playerId] : null;
  const lastGain =
